...
|P7: 从零开始，用代码构建 GPT|2026.8.6~ |- nn.Embedding层随机初始化<br/>- optimizer.zero_grad(set_to_none = True),优化器设置梯度为0或者None的区别<br/>- 自注意力机制中的数学技巧(不同时间步求均值改为矩阵乘法, 为什么B批次维度会放在第一位, 多维矩阵计算)|
|P8: GPT现状(BRK216HFS)|
|P9: 构建 GPT 分词器|
